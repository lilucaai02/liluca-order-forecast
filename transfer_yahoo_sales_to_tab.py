#!/usr/bin/env python3
"""
日次Yahoo販売推移シート → 各タブの「Yahoo販売実績」行への転記（汎用）。

タブ別の設定は tab_blocks_config.py の TAB_BLOCKS で管理。
ブロックに `yahoo_sales_row` が定義されていない場合はスキップ。

Yahoo SKU は「item_id:sub_code」形式（または item_id のみ）。
商品コードへの集計は正規化マッチで完全一致するもの。

使い方:
  python3 transfer_yahoo_sales_to_tab.py --tab "DS-01 (在庫) "
  python3 transfer_yahoo_sales_to_tab.py --tab "マウスピース(在庫)" --from-date 2026-07-15 --to-date 2026-07-21
...
def normalize_sku(sku: str) -> str:
    ...
    Yahoo SKUは item_id:sub_code 形式。商品コードと対応するのは末尾側
    （sub_code があればそれ、なければ item_id そのもの）。
    """
    s = sku.lower().strip()
    s = s.replace("（", "(").replace("）", ")")
    s = re.sub(r"\([^)]*\)", "", s)
...
def read_sales_from_source(gc, src_spreadsheet_id: str,
    ...
    """{(正規化SKU, 日付): 全アカウント合算 qty} と 不明な(正規化SKU,日付)の集合。
...
def main():
    ...
    if not blocks_with_yahoo:
        print(f"⚠ {args.tab} に yahoo_sales_row 定義のブロックがありません", file=sys.stderr)
        return
    ...
    print(f"=== [{args.tab}] Yahoo販売実績 転記 [{args.from_date} ～ {args.to_date}] ===",
          file=sys.stderr)
    ...
    total_cells = len(blocks_with_yahoo) * len(dates)
    ...
    for blk in blocks_with_yahoo:
        for d_str in dates:
            ...
            updates.append({"range": f"{col}{blk['yahoo_sales_row']}", "values": [[qty]]})
