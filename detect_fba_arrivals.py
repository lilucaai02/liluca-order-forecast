...
def fetch_amazon_inbound(settings: Settings) -> Tuple[Dict[str, int], List[str]]:
    ...
    for acc in settings.get_accounts():
        try:
            ...
            continue
        ok += 1
        # 在庫は FNSKU 単位で管理されている。同じASINでも FNSKU が違えば
        # 別の在庫なので合算する。FNSKU が同じSKUは同じ在庫を指しているので
        # 1つだけ採る (足すと二重計上になる)。
        ...
        for it in items:
            ...
            k = (it.asin, it.fn_sku or f"sku:{it.seller_sku}")
            per_fn[k] = max(per_fn.get(k, 0), n)
        for (asin, _fn), n in per_fn.items():
